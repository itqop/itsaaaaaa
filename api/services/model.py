...
def get_n_best(arg_sphere, arg_numbest):
  data_bez_con=pd.read_csv(model_dir.joinpath('model_dataset.csv'))
  ...
  cb.load_model(model_dir.joinpath('modelcb.cbm'))
  ans_d=data_bez_con.copy()
  ans_d[10]=[arg_sphere for i in range(ans_d.shape[0])]
  a=pd.read_csv(model_dir.joinpath('data_with_concur.csv'))
